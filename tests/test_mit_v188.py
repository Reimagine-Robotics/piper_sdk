...
def test_firmware_parser_strips_glued_build_date():
    interface = make_interface(b"S-V1.8-5230101")

    assert interface.GetPiperFirmwareVersion() == "S-V1.8-5"


def test_dated_pre_v188_firmware_does_not_use_12bit_frame():
    # regression: greedy patch parse read the glued date into the patch, so a
    # dated 1.8-5 arm classified as >= 1.8-8 and was sent the 12-bit frame.
    interface = make_interface(b"S-V1.8-5230101")
...
def test_dated_v188_firmware_uses_12bit_frame():
    interface = make_interface(b"S-V1.8-8230101")
...
def test_unexpected_date_width_falls_back_to_8bit_frame():
    # 8-digit date is outside the known 6-digit format: rather than let the
    # patch swallow digits and misclassify, the parse fails and the safe 8-bit
    # frame is used.
    interface = make_interface(b"S-V1.8-520230101")

    assert interface.GetPiperFirmwareVersion() == -0x4AF
