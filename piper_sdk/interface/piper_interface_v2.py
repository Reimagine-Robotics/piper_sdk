...
# AgileX firmware strings are "S-V<major>.<minor>-<patch>" optionally followed by
# a glued 6-digit YYMMDD build date and trailing buffer bytes. The patch is
# bounded to 1-2 digits and the date is a fixed 6-digit run so the date never
# bleeds into the patch (e.g. "S-V1.8-5230101" -> patch 5, not 5230101). An
# unexpected date width fails to match and falls back to the safe 8-bit frame
# rather than misclassifying the patch.
_PIPER_FIRMWARE_RE = re.compile(rb"S-V(\d+)\.(\d+)-(\d{1,2}?)(?:\d{6})?(?=\D|$)")
...
class C_PiperInterface_V2():
    ...
    def GetPiperFirmwareVersion(self):
        ...
        with self.__firmware_data_mtx:
            version_match = _PIPER_FIRMWARE_RE.search(self.__firmware_data)
            if version_match is None:
                return -0x4AF  # 没有找到以 S-V 开头的字符串
            major, minor, patch = (int(g) for g in version_match.groups())
            return f"S-V{major}.{minor}-{patch}"  # 返回找到的固件版本字符串
    ...
    def __PiperFirmwareTuple(self):
        ...
        with self.__firmware_data_mtx:
            match = _PIPER_FIRMWARE_RE.search(self.__firmware_data)
            if match is None:
                return None
